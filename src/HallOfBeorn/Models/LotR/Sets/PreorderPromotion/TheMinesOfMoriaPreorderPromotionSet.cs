...
namespace HallOfBeorn.Models.LotR.Sets.PreorderPromotion
{
    ...
    {
        private const string setName = "The Mines of Moria Preorder Promotion";
        private const string emptySetName = "";

        public static CardSet Create() {

            var cs = CardSet.Create(setName, "TMoM-PRM", Models.SetType.Preorder_Promotion, "Custom Scenario Kit", 7);

            cs.addQuest("To the West-door", EncounterSet.TheMinesOfMoria, 1, 'A', 5)
            ...
            cs.addEnemy("The Watcher", EncounterSet.TheMinesOfMoria, 50, 2, 4, 1, 4)
                .WithUnique()
                .WithTraits("Creature.")
            ...
            cs.addLocation("Chamber of Mazarbul", EncounterSet.TheMinesOfMoria, Card.VALUE_X, 5)
                .WithUnique()
            ...
            cs.addLocation("Many-pillared Hall", emptySetName, 1, 4)
