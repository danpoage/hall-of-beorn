...
namespace HallOfBeorn.Models.LotR.Sets.PreorderPromotion
{
    ...
    {
        private const string setName = "Escape from Khazad-dûm Preorder Promotion";
        private const string emptySetName = "";

        public static CardSet Create() {

            var cs = CardSet.Create(setName, "EfKD-PRM", Models.SetType.Preorder_Promotion, "Custom Scenario Kit", 8);

            cs.addQuest("Search for an Exit", EncounterSet.EscapeFromKhazadDum, 2, 'A', 20)
            ...
            cs.addEnemy("Durin's Bane", EncounterSet.EscapeFromKhazadDum, 50, 4, 8, 3, 15)
                .WithUnique()
                .WithTraits("Balrog.")
            ...
            cs.addEnemy("Black Bats", emptySetName, 23, 1, 3, 1, 2)
            ...
            cs.addLocation("Lightless Grotto", emptySetName, 2, 3)
