...
namespace HallOfBeorn.Models.LotR.Sets.TheLordOfTheRings
{
    public class TheMountainOfFire : CardSet
    {
        ...
        protected override void Initialize()
        {
            ...
            addCard(LotRCard.Location("Orc-hold", string.Empty, setMountDoom, 3, 4)
                .WithTraits("Mordor.")
            ...
            addCard(LotRCard.Location("Mordor Road", string.Empty, setMountDoom, 1, 5)
                ...
                .WithFlavor("They climbed on to the causeway and trudged along, down the hard cruel road that led to the Dark Tower itself.\r\n-The Return of the King")
                .WithEyeIcon()
                .WithEasyModeQuantity(1)
                .WithInfo(45, 2, Artist.Stanislav_Dikolenko));
            addCard(LotRCard.Location("Plain of Gorgoroth", string.Empty, setMountDoom, 2, 4)
                .WithTraits("Mordor.")
                .WithText("If a player has failed a Fortitude test this round, Plain of Gorgoroth gets +2 Threat.\r\nTravel: The first player makes a Fortitude test to travel here. If he fails this test, raise each player's threat by 4.")
                .WithFlavor("There smokes trailed on the ground and lurked in hollows, and fumes leaked from fissures in the earth.\r\n-The Return of the King")
                .WithInfo(46, 4, Artist.Igor_Artyomenko));
            addCard(LotRCard.Location("Brake of Thorns", string.Empty, setMountDoom, 3, 3)
                .WithTraits("Mordor.")
                .WithText("While Brake of Thorns is in the staging area, add (|) to each Fortitude test.\r\nForced: After Brake of Thorns becomes the active location, each player assigns X damage among characters he controls, where X is the tens digit of his threat dial.\r\n(|)")
                .WithFlavor("They had a struggle to get out of the thicket. The thorns and briars were as tough as wire and clinging as claws.\r\n-The Return of the King")
                .WithEyeIcon()
                .WithEasyModeQuantity(2)
                .WithInfo(47, 3, Artist.Leanna_Crossan));
            addCard(LotRCard.Location("Old Water-course", string.Empty, setMountDoom, 2, 5)
                .WithTraits("Mordor.")
                .WithText("While Old Water-course is the active location, it gains: \"Response: When Old Water-course is explored, reduce each player's threat by 5.\"\r\nTravel: Each player must exhaust a hero he controls to travel here.")
                .WithFlavor("Clambering a few steps up he found a tiny stream of dark water that came out from the hill-side and filled a little bare pool...\r\n-The Return of the King")
                .WithVictoryPoints(2)
                .WithInfo(48, 1, Artist.Leanna_Crossan));
            addCard(LotRCard.Enemy("Gollum", string.Empty, setMountDoom, 99, 2, 2, 2, 5)
                .WithUnique()
                .WithTraits("Gollum.")
                .WithKeywords("Surge.")
                .WithText("Immune to non-Fellowship player card effects.\r\nGollum engages the first player.\r\n*Forced: After Gollum is discarded during a Fortitude test, put him into play engaged with the first player. Then, choose an encounter card discarded from this test and ignore its (|) icons.")
                .WithInfo(49, 1, Artist.Martin_de_Diego_Sadaba));
            addCard(LotRCard.Enemy("Ringwraith of Sauron", string.Empty, setMountDoom, 88, 3, 6, 3, 10)
                .WithTraits("Nazgûl.")
                ...
                .WithFlavor("...with a storm of wings they hurtled southwards to Mount Doom.\r\n-The Return of the King")
                .WithEyeIcon()
                .WithEyeIcon()
                .WithEasyModeQuantity(1)
                .WithInfo(50, 2, Artist.Alexander_Kozachenko));
            addCard(LotRCard.Enemy("Orc Captain", string.Empty, setMountDoom, 55, 3, 5, 4, 6)
                .WithTraits("Orc.", "Uruk.")
                .WithText("Forced: After Orc Captain engages you, make a Fortitude test. If you fail this test, reveal the top card of the encounter deck.\r\n(|) (|)")
                .WithShadow("Shadow: If this attack destroys a character, return attacking enemy to the staging area.")
                .WithEyeIcon()
                .WithEyeIcon()
                .WithEasyModeQuantity(1)
                .WithInfo(51, 2, Artist.Carlos_Palma_Cruchaga));
            addCard(LotRCard.Enemy("Big Uruk", string.Empty, setMountDoom, 44, 2, 4, 3, 5)
                .WithTraits("Orc.", "Uruk.")
                .WithText("Forced: After Big Uruk engages you, either raise your threat by 4, or Big Uruk makes an immediate attack against you.\r\n(|)")
            ...
            addCard(LotRCard.Enemy("Little Snuffler", string.Empty, setMountDoom, 33, 1, 3, 2, 3)
                ...
                .WithEyeIcon()
                .WithEasyModeQuantity(2)
                .WithInfo(53, 4, Artist.Borja_Pindado));
            addCard(LotRCard.Treachery("'Wicked Master!'", string.Empty, setMountDoom)
                .WithTraits("Gollum.")
                .WithText("When Revealed: Gollum makes an immediate attack against the first player. If Gollum is not in play, search the encounter deck and discard pile for Gollum and put him into play engaged with the first player.\r\n(|) (|) (|)")
                .WithShadow("Shadow: If attacking enemy is Gollum, he makes an additional attack after this one.")
                .WithEyeIcon().WithEyeIcon().WithEyeIcon()
                .WithEasyModeQuantity(0)
                .WithInfo(54, 1, Artist.Jennifer_Hrabota_Lesser));
            addCard(LotRCard.Treachery("The Lidless Eye", string.Empty, setMountDoom)
                .WithKeywords("Peril.")
                .WithText("When Revealed: Make a Fortitude test. If The One Ring is exhausted, add (|) (|) to the test. If you fail this test, raise your threat by 10.\r\n(|) (|)")
                .WithShadow("Shadow: Attacking enemy gets +1 Attack (+2 Attack instead if you failed a Fortitude test this round).")
                .WithEyeIcon().WithEyeIcon()
                .WithEasyModeQuantity(3)
                .WithInfo(55, 4, Artist.DinoDrawing));
            addCard(LotRCard.Treachery("Foul Fumes", string.Empty, setMountDoom)
                .WithText("When Revealed: Each player makes a Fortitude test. If you fail this test, deal 1 damage to each character you control. Until the end of the round, treat the printed text box of each damaged character as if it were blank (except for Traits).\r\n(|) (|)")
                .WithShadow("Shadow: If you failed a Fortitude test this round, this attack is considered undefended.")
                .WithEyeIcon().WithEyeIcon()
                .WithEasyModeQuantity(1)
                .WithInfo(56, 2, Artist.Borja_Pindado));
            addCard(LotRCard.Treachery("Heavy and Tired", string.Empty, setMountDoom)
                .WithText("When Revealed: Attach Heavy and Tired to the hero with the most attachments and exhaust that hero. (Counts as a Condition attachment with the text: \"Limit 1 per hero. Attached hero cannot commit to Fortitude tests. Forced: After you Fail a Fortitude test, exhaust attached hero and discard each non-objective attachment on it.\")\r\n(|)")
                .WithFlavor("\"This extra weight is killing me. It must go.\"\r\n-Frodo Baggins, The Return of the King")
                .WithEyeIcon()
                .WithEasyModeQuantity(1)
                .WithInfo(57, 2, Artist.Borja_Pindado));
            ...
            addCard(LotRCard.Quest("The Land of Mordor", 1, setMountDoom, 4)
                ...
                .WithIncludedEncounterSets(EncounterSet.DeepShadows)
