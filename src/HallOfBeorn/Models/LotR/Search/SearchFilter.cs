...
namespace HallOfBeorn.Models.LotR.Search
{
    ...
    {
        public SearchFilter(Func<SearchViewModel, LotRCard, bool> check, float score, string description)
            : this(check, score, 0f, description)
        {
        }

        public SearchFilter(Func<SearchViewModel, LotRCard, bool> check, float score, float miss, string description)
            : this(check, score, miss, description, true)
        {
        }

        public SearchFilter(Func<SearchViewModel, LotRCard, bool> check, float score, float miss, string description, bool isWeighted)
        {
            _check = check;
            _score = score;
            _miss = miss;
        ...
        private Func<SearchViewModel, LotRCard, bool> _check;
        ...
        */
        ...
        public string Description(SearchViewModel search, LotRCard card)
        {
        ...
        public float Score(SearchViewModel search, LotRCard card)
        {
            if (_filters != null)
            {
                var bestScore = 0f;
        ...
        public static float WeightedScore(LotRCard card, float score)
        {
            var weight = 0;
