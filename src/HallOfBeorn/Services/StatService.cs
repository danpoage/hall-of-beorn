...
namespace HallOfBeorn.Services
{
    public class StatService
    {
        ...
        private void Analyze(Card card)
        {
        ...
        {
            return cards.Where(x => !string.IsNullOrEmpty(x.ResourceCostLabel)).OrderBy(x => x.ResourceCost).Select(x => x.ResourceCostLabel).Distinct();
        }

        public IEnumerable<string> ThreatCosts()
        {
            return cards.Where(x => !string.IsNullOrEmpty(x.ThreatCostLabel)).OrderBy(x => x.ThreatCost).Select(x => x.ThreatCostLabel).Distinct();
        }

        public IEnumerable<string> EngagementCosts()
        {
            return cards.Where(x => !string.IsNullOrEmpty(x.EngagementCostLabel)).OrderBy(x => x.EngagementCost).Select(x => x.EngagementCostLabel).Distinct();
        }
