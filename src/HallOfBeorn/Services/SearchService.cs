...
namespace HallOfBeorn.Services
{
    public class SearchService
    {
        ...
        public IEnumerable<CardScore> Search(SearchViewModel model)
        {
            ...
            {
                if (filters.Count == 0 && model.SetType.Value == SetType.OFFICIAL && !model.IsAdvancedSearch())
                {
                    limit = 12;
                    model.Sort = Sort.Set_Number;
                }

                filters.Add(new WeightedSearchFilter((s, c) => { return (s.SetType == SetType.OFFICIAL && c.CardSet.SetType != SetType.CUSTOM) || (s.SetType.Value == c.CardSet.SetType); }, 50, "Has Set Type '" + model.SetType + "'"));
            }
            ...
            var sortedResults = sortService.Sort(model, filters, results, limit);
